...
class PlotAccountActivity():
    data = pd.read_csv("../micro_world.csv")
    ph_data =  data[data['economy']=='Philippines']

    # def __init_subclass__(self):
    #     self.data = pd.read_csv("../micro_world.csv")
    #     self.ph_data =  self.data[self.data['economy']=='Philippines']
    ...
    def has_an_account(self):
        ...
        self.ph_data['Number of accounts'] = self.ph_data.apply(classify_account , axis = 1)
        subset_account = self.ph_data[['wpid_random','account','Number of accounts']]
    ...
    def plot_account_activity(self):
    
        ## data wrangling
        fin_activity = self.ph_data[['wpid_random', 'account', 'account_fin', 'fin9', 'fin10']]
