...
def reasons_no_account(ph_data):
    ...
    reasons =reasons.sort_values('value', ascending=True).head(10).reset_index(drop=True)
    reasons['reason'] = ['FI are too far away', 'FS are too expensive', 'No necessary documentation', 'No trust in FI', 'Religious reasons', 'Not enough money to use FI',
               'Family already has account', 'No need for FS' ]
